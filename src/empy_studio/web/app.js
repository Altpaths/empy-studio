...
const t = {
  fa: {
    ...
    newProject: "پروژه‌ی دیگر", savedProjects: "پروژه‌های ذخیره‌شده", noProject: "در این workspace هنوز پروژه‌ای وارد نشده است.", projectListHelp: "این بخش فقط پروژه‌هایی را نشان می‌دهد که قبلاً در همین workspace وارد کرده‌اید.", importFirstProject: "ورود اولین پروژه", reimportProject: "ورود دوبارهٔ پروژه", projectUnavailable: "مسیر این پروژه دیگر وجود ندارد؛ پوشه یا ZIP را دوباره وارد کنید.", noTask: "هنوز تیکتی ثبت نشده است.", engine: "وضعیت Codex",
  ...
  en: {
    ...
    newProject: "Another project", savedProjects: "Saved projects", noProject: "No project has been imported in this workspace yet.", projectListHelp: "This section only shows projects previously imported into this workspace.", importFirstProject: "Import the first project", reimportProject: "Re-import project", projectUnavailable: "This project's path is no longer available; choose its folder or ZIP again.", noTask: "No tickets have been registered.", engine: "Codex status",
...
function projectList() {
  ...
  if (!projects.length) {
    return `<div class="empty-projects"><p class="muted">${text().noProject}</p><p class="muted">${text().projectListHelp}</p><div class="actions"><button type="button" class="primary" data-action="choose-zip">${text().importFirstProject}</button><button type="button" class="secondary" data-action="choose-folder">${text().folder}</button></div></div>`;
  }
