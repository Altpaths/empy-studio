...
function sanitizeFailureMemoryText(value, limit = 320) {
  let result = String(value ?? "").replace(/[\u0000-\u001f\u007f]/g, " ").replace(/\s+/g, " ").trim();
  if (!result) return "";
  result = result
    .replace(/(?:sk|pk)-[A-Za-z0-9_-]{12,}/g, "[redacted]")
    .replace(/(?:gh[pousr]_[A-Za-z0-9_]{12,}|xox[baprs]-[A-Za-z0-9-]{12,})/g, "[redacted]")
    .replace(/\b(api[_ -]?key|token|password|secret|authorization)\s*[:=]\s*[^\s,;]+/gi, "$1: [redacted]")
    .replace(/(^|[\s(])\/(?:Users|private|tmp|var|home|opt|Volumes|Applications|Library|System|etc|usr|root)[^\s<>"')}]*/gi, "$1[project path]")
    .replace(/(^|[\s(])[A-Za-z]:\\[^\s<>"')}]*/g, "$1[project path]");
  return result.length > limit ? `${result.slice(0, Math.max(0, limit - 1)).trimEnd()}…` : result;
}
function failureMemoryEntry(value) {
  if (typeof value === "string") return {summary: sanitizeFailureMemoryText(value)};
  if (!value || typeof value !== "object" || Array.isArray(value)) return null;
  const summary = sanitizeFailureMemoryText(value.summary || value.compact_summary || value.title || value.finding || value.message);
  const action = sanitizeFailureMemoryText(value.action || value.next_step || value.required_action, 260);
  return summary || action ? {summary, action} : null;
}
function normaliseFailureMemory(value) {
  if (!value || typeof value !== "object" || Array.isArray(value)) return null;
  const rawMatched = value.matched;
  const matched = Array.isArray(rawMatched)
    ? rawMatched
    : rawMatched && typeof rawMatched === "object"
      ? [rawMatched]
  ...
  [...matched.slice(0, 4), ...summaries.slice(0, 4)].forEach(item => {
    const entry = failureMemoryEntry(item);
    if (!entry) return;
    const key = `${entry.summary}\u0000${entry.action}`;
...
function renderFailureMemory(memory) {
  const normalized = normaliseFailureMemory(memory);
  if (!normalized) return "";
  const count = normalized.openCount
...
function statusLabel(value) {
  const labels = language === "fa" ? {completed:"کامل شد", pass:"موفق", failed:"ناموفق", fail:"ناموفق", cancelled:"لغو شد", timed_out:"پایان زمان", unavailable:"در دسترس نیست", skipped:"به‌دلیل توقف مرحلهٔ قبل اجرا نشد", running:"در حال اجرا", waiting:"در انتظار", not_run:"اجرا نشده", ready_for_export:"آماده تولید ZIP", verified_no_change:"تغییر لازم نبود؛ Verification موفق شد", awaiting_review:"در انتظار تأیید تغییرات", blocked:"مسدود", exported:"خروجی تولید شد"} : {completed:"Completed", pass:"Passed", failed:"Failed", cancelled:"Cancelled", timed_out:"Timed out", unavailable:"Unavailable", skipped:"Not run (previous step stopped)", running:"Running", waiting:"Waiting", not_run:"Not run", ready_for_export:"Ready to create ZIP", verified_no_change:"Verified; no change was needed", awaiting_review:"Waiting for change approval", blocked:"Blocked", exported:"Export created"};
...
function renderReleaseGateExplanation(gate, verification, review) {
  ...
  if (gate?.status === "verified_no_change") {
    return `<section class="release-gate-explanation review-waiting" role="status"><strong>${text().nextStep}</strong><p>${escapeHtml(text().verifiedNoChange)}</p></section>`;
  }
...
function renderRouteAccounting(report) {
  const ledger = report.task_ledger; const route = report.route_report; if (!ledger && !route) return "";
  const fa = language === "fa";
  const attempts = (route?.attempts || []).map(item => `${escapeHtml(item.provider_id || "?")} · ${escapeHtml(item.status || "?")}${item.failure_class ? ` · ${escapeHtml(item.failure_class)}` : ""}`).join(" → ");
  const unknown = (ledger?.unknown_operation_ids || []).length;
  const routeText = route ? `${route.switched ? (fa ? "مسیر جایگزین استفاده شد" : "Fallback route used") : (fa ? "تعویض مسیر انجام نشد" : "No route switch")} · ${escapeHtml(route.reason || "")}${attempts ? ` · ${attempts}` : ""}` : "";
  const ledgerText = ledger ? `${fa ? "ثبت‌شده / رزروشده / باقی‌مانده" : "Charged / reserved / remaining"}: ${Number(ledger.charged_tokens || 0).toLocaleString()} / ${Number(ledger.reserved_tokens || 0).toLocaleString()} / ${Number(ledger.remaining_tokens || 0).toLocaleString()}${unknown ? ` · ${fa ? "مصرف نامعلوم" : "Unknown usage"}: ${unknown}` : ""}` : "";
  return `<details class="verification-details routing-details"><summary>${fa ? "حساب توکن و مسیر اجرا" : "Token ledger and route"}</summary><p class="muted">${escapeHtml(ledgerText)}</p>${routeText ? `<p class="muted">${routeText}</p>` : ""}</details>`;
}
...
async function handleAction(action, target) {
  switch (action) {
    ...
    case "save-model-route": {
      const fallbackValue = document.querySelector("#model-route-fallbacks").value;
      const route = {mode: document.querySelector("#model-route-mode").value, base_url: document.querySelector("#model-route-url").value.trim(), model: document.querySelector("#model-route-model").value.trim(), fallback_models: fallbackValue.split(",").map(value => value.trim()).filter(Boolean), env_key: document.querySelector("#model-route-env").value.trim() || null, allow_paid: document.querySelector("#model-route-paid").checked};
