...
def test_release_workflow_builds_the_persistent_macos_app() -> None:
    workflow = (PROJECT_ROOT / ".github" / "workflows" / "release.yml").read_text(
        encoding="utf-8"
    )

    assert "--clean-workspace" not in workflow
    build_start = workflow.index("python scripts/build_macos_app.py")
    build_end = workflow.index("test -x", build_start)
    assert "--clean-workspace" not in workflow[build_start:build_end]
